...
public class GameFragment extends BaseFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        BoardConfiguration boardConfiguration = Shared.engine.getActiveGame().boardConfiguration;
        ImageView foreground = Shared.activity.findViewById(R.id.foreground);
        if (boardConfiguration.getImpairment() == Impairment.VISUAL_BLUR) {
            foreground.setVisibility(View.VISIBLE);
            foreground.setImageResource(R.drawable.blur);
            foreground.setAlpha(0.98f);
        } else {
    ...
    private void buildBoard() {
        ...
        int time = game.boardConfiguration.getTime();
        setTime(time);
